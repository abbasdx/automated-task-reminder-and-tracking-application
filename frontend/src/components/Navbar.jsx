...
import { LayoutDashboard, LineChart, LogOut, Menu, X, BadgeCheck, AlertCircle } from "lucide-react";
...
export default function Navbar() {
  ...
  // FIXED: Removed async/await. Just open the modal instantly. The modal handles the API call.
  ...
  return (
    <>
                ...
                {dropdownOpen && (
                  <div className="absolute right-0 mt-4 w-56 origin-top-right rounded-2xl border border-white/[0.08] bg-[#0a0e17]/95 p-2 shadow-2xl backdrop-blur-2xl animate-in fade-in zoom-in-95 slide-in-from-top-2">
                    ...
                    <div className="px-3 py-3 flex flex-col gap-1">
                      <p className="truncate text-sm font-semibold text-slate-200 flex items-center gap-1.5">
                        {user?.name}
                        {user?.emailVerified && <BadgeCheck size={14} className="text-emerald-400 shrink-0" />}
                      </p>
                      <p className="truncate text-xs text-slate-500">{user?.email}</p>
                      
                      {/* Desktop Verify Button */}
                      {!user?.emailVerified && (
                        <button onClick={handleVerifyClick} className="mt-1.5 inline-flex w-fit cursor-pointer items-center gap-1.5 rounded-md border border-amber-500/20 bg-amber-500/10 px-2 py-1 text-[0.65rem] font-medium text-amber-400 transition-colors hover:bg-amber-500/20">
              ...
              {loggedIn ? (
                <div className="flex flex-col gap-1">
                  <div className="flex flex-col gap-1 px-4 py-2">
                    ...
                    {!user?.emailVerified && (
                      <button onClick={handleVerifyClick} className="mt-1.5 inline-flex w-fit cursor-pointer items-center gap-1.5 rounded-md border border-amber-500/20 bg-amber-500/10 px-2 py-1 text-xs font-medium text-amber-400 transition-colors hover:bg-amber-500/20">
                        <AlertCircle size={14} /> Verify Email
                      </button>
                    )}
                  </div>
      ...
      {/* Attach Modal globally to Navbar */}
      <OtpModal 
        isOpen={showOtpModal} 
        onClose={() => setShowOtpModal(false)} 
        user={user} /* FIXED: Passed user object instead of just email string */
        onSuccess={() => {
          setShowOtpModal(false);
          // Auto-update UI without refresh
          setUserState({...user, emailVerified: true}); 
        }} 
      />
    </>
  );
